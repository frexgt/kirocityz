if SERVER then
    ...
    local function wrap(name, recv)
        return function(len, ply)
            ...
            local now = CurTime()
            ply._ok_net_windows = ply._ok_net_windows or {}
            local w = ply._ok_net_windows[name]
            if not w or now - w.t >= 1 then
                w = { t = now, c = 0 }
                ply._ok_net_windows[name] = w
            end
            w.c = w.c + 1
    ...
    if not net._ok_original_Receive then
        net._ok_original_Receive = net.Receive
        net.Receive = function(name, handler)
            return net._ok_original_Receive(name, wrap(name, handler))
        end
    end
end